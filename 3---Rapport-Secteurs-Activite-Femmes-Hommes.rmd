...
# Lecture des données
```{r}
x_matrix <- read.csv("csv/generated/year-activity-men-women-format.csv", header = T, sep = ";", row.names = 1)
```
## Nombre de colonne
```{r}
ncol(x_matrix)
...
On observe que les secteur de l'industrie est fortement négativement corrélé avec la composante principale 1 tandis que que les deux secteurs du tertiaire lui sont fortement corrélés. De plus, la construction lui est également assez correlée, alors que l'agriculture lui est légèrement négativement correlée.

On peut donc interpréter la composante princiale 1 comme le fait d'être un secteur qui fournit un service plus qu'un secteur qui manufacture des produits.

Malheureusement, nous n'avons pu déterminer d'interprétation satisfaisante pour la composante principales 2.
