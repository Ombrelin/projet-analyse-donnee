...
Cependant, on peut également constater que la variable *homme* est négativement corrélée à la seconde composante princpale, et que la variable *femme* est quant à elle légerement corrélée à cette dernière.

Etant donné que la quantité d'information portée par la première composante principale est bien supérieure à celle portée par la seconde.
Ainsi, on peut dire que la première composante principale peut être analysée comme **La quantité de personnes employés cette année**
...
abline(h = 0, v = 0)
```
A l'observation de ce graphique du nuage des individus, on peut observer quatres groupes d'années :
- Le groupe 1 : Les années 1992, 1993, 1994, 1995, 1996, 1997

Ce groupe présente des valeurs relativement élevées dans la composante principale 2 et des valeurs plus faibles dans la composantes principale 1.
...
On peut donc dire que ces années sont celles ou ont été employées le moins de femmes mais beaucoup de personnes.
